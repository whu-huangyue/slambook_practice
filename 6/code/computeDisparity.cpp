 ...
 * @param T21
 */
vector<Eigen::Vector2d, Eigen::aligned_allocator<Eigen::Vector2d>>  DirectPoseEstimationMultiLayer(
        const cv::Mat &img1,
        const cv::Mat &img2,
 ...
 * @param T21
 */
vector<Eigen::Vector2d, Eigen::aligned_allocator<Eigen::Vector2d>>  DirectPoseEstimationSingleLayer(
        const cv::Mat &img1,
        const cv::Mat &img2,
        const VecVector2d &px_ref,
        const vector<double> depth_ref,
...
int main(int argc, char **argv) {
    ...
    cv::Mat img = cv::imread(right_file,  0);
//    goodProjection = DirectPoseEstimationSingleLayer(left_img, img, pixels_ref, depth_ref, T_cur_ref);    // first you need to test single layer
    goodProjection = DirectPoseEstimationMultiLayer(left_img, img, pixels_ref, depth_ref, T_cur_ref);
...
vector<Eigen::Vector2d, Eigen::aligned_allocator<Eigen::Vector2d>> DirectPoseEstimationMultiLayer(
    ...
    for (int level = pyramids - 1; level >= 0; level--) {
        ...
        cy = cyG * scales[level];
        // END YOUR CODE HERE
        goodProjection = DirectPoseEstimationSingleLayer(pyr1[level], pyr2[level], px_ref_pyr, depth_ref, T21);
    }
    return goodProjection;
}
